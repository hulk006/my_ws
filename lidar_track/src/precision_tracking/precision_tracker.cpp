    ...
    ScoredTransforms<ScoredTransformXYZ>* scored_transforms)
{
  ...
  // Down-sample the previous points.//为了加速
  pcl::PointCloud<pcl::PointXYZI>::Ptr previous_model_downsampled(
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr down_sampled_current(
        new pcl::PointCloud<pcl::PointXYZI>);
  down_sampler_.downSamplePoints(
    ...
    std::pair <double, double>* zRange) const
{
  ...
  pcl::PointXYZI max_pt_prev;
  pcl::PointXYZI min_pt_prev;
  ...
  // Find the min and max of the previous points.
  pcl::PointXYZI max_pt_curr;
  pcl::PointXYZI min_pt_curr;
...
Eigen::Matrix4f PrecisionTracker::estimateAlignmentCentroidDiff(
    const pcl::PointCloud<pcl::PointXYZI>::ConstPtr& curr_points,
    const pcl::PointCloud<pcl::PointXYZI>::ConstPtr& prev_points) const
{
