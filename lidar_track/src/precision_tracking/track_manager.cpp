...
void Track::insertFrame(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud,
                       double timestamp)
{
...
Frame::Frame(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud,
    double timestamp) :
...
Eigen::Vector3f Frame::getCentroid() {
  ...
  *centroid_ /= (float)cloud_->points.size();
...
void  deserializePointCloud(std::istream& istrm,
  ...
  for (size_t i = 0; i < point_cloud.size(); ++i) {
    pcl::PointXYZI& pt = point_cloud[i];
  }
}
...
void serializePointCloud(
    const pcl::PointCloud<pcl::PointXYZI> cloud, ostream& out) {
  pcl::PCLPointCloud2 msg;
...
bool cloudsEqual(const pcl::PointCloud<pcl::PointXYZI>& c1,
    const pcl::PointCloud<pcl::PointXYZI>& c2) {
