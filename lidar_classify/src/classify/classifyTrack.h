...
{
public:
    ClassifyTrack(ros::NodeHandle node, ros::NodeHandle private_nh,
                  std::vector<std::string> modelRangeFiles);
    ~ClassifyTrack();
 //  MultiClassifierSVM mcf;
private:
    void classifyTrackCallback(const lidar_msg::clusterArray &msg);
    void classifySeg(Robosense::Tracking& MM);
    void classifyBayesFilter(Robosense::Tracking& MM);
    void initMarker(visualization_msgs::Marker& marker);
    void generateSegfile(Robosense::Tracking &MM);
    int labelByProb(std::vector<float> &prob);
    Robosense::ClassifierSVM mcf;
