...
public:
    ...
    ShapeEstimator(pcl::PointCloud<pcl::PointXYZI>::Ptr  ptr_);
    ...
    float distance(pcl::PointXYZI p);
    ...
    std::vector<cv::Point2f> convert2CVPoint_P(pcl::PointCloud<pcl::PointXYZI>::Ptr in);
    std::vector<cv::Point2f> findNearest_P(pcl::PointCloud<pcl::PointXYZI>::Ptr in);
    //std::vector<pcl::PointCloud<pcl::PointXYZL>::Ptr > pointCloudSeparate(pcl::PointCloud<pcl::PointXYZL>  labeled_pc, int label_num);
    cv::Point2f findLimit_Z(pcl::PointCloud<pcl::PointXYZI>::Ptr in);//x = max  y = min
